...
function loca_test({ texte, repondre }) {
    ...
    if (typeof texte === "string" && texte.toLowerCase().startsWith("*`💠interface`*\n▔▔▔▔▔▔▔▔▔▔▔▔▔■■■■■\n🌍position:")) {
        ...
        if (match) {
            const startPosition = parseInt(match[1], 10); // Position de départ
            const guillemets = match[2]; // ">" ou ">>"
            const endPosition = parseInt(match[3], 10); // Position d'arrivée

            const distance = Math.abs(endPosition - startPosition); // Distance parcourue
            ...
            // Récupère les lieux de départ et d'arrivée
